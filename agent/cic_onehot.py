...
class CICAgent(DDPGAgent):
    ...
    def __init__(self, update_skill_every_step, skill_dim, scale, 
        ...
        self.ft_not_finished = [True for _ in range(self.skill_dim)]
        self.ft_returns = np.zeros(self.skill_dim, dtype=np.float32)
    ...
    def init_meta(self):
        if not self.reward_free:
            if not any(self.ft_not_finished):
                ...
                new_z = np.zeros(self.skill_dim, dtype=np.float32)
                new_z[new_z_ind] = 1.0
                return OrderedDict(skill=new_z)
            else:
                if not hasattr(self, "_last_meta_idx"):
                    self._last_meta_idx = -1
                self._last_meta_idx = (self._last_meta_idx + 1) % self.skill_dim
                idx = self._last_meta_idx
                skill = np.zeros(self.skill_dim, dtype=np.float32)
                skill[idx] = 1.0
                return OrderedDict(skill=skill)
        else:
            if not hasattr(self, "_last_meta_idx"):
                self._last_meta_idx = -1
            self._last_meta_idx = (self._last_meta_idx + 1) % self.skill_dim
    ...
    def update_meta_ft(self, meta, global_step, time_step):
        if not any(self.ft_not_finished):
            ...
            z_ind = int(np.argmax(meta['skill']))
            self.ft_returns[z_ind] += time_step.reward

            self.ft_not_finished[z_ind] = False
            candidates = np.flatnonzero(self.ft_not_finished)

            if candidates.size == 0 and self.pick_all_round == 0:
                new_z_ind = self.ft_returns.argmax()
            elif candidates.size == 0 and self.pick_all_round != 0:
                self.pick_all_round -= 1
                self.ft_not_finished = [True for _ in range(self.skill_dim)]
                candidates = np.flatnonzero(self.ft_not_finished)
                new_z_ind = int(np.random.choice(candidates))
