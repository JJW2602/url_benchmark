...
Debugging: 
Initialize workspace.( O )
MI measurement function.(  )
    Collecting samples for each skill z.( O )

        Replay buffer 안에 잘 담기는지.
        z별 sample을 할 때 잘 sample하는지
    Estimating E[log d(s)].(  )
    Estimating E[log d(s|z)].(  )
...
class Workspace:
    ...
    def measure_mi(self):
        ...
        rep_dim = rep.shape[-1]
        with torch.no_grad():
            log_d_s_B = (-self.pbe(rep) * rep_dim).squeeze(-1)  # [B]
            E_log_d_s = log_d_s_B.mean()                        # scalar
            print(f"E[log d(s)]={E_log_d_s.item():.6f}")
        # ===========================================
        ...
        for z in range(self.agent.skill_dim):
            ...
            # 2) log d(s|z) 추정
            log_d_s_given_z = (-self.pbe(rep_b) * rep_b_dim).squeeze(-1)   # (B,)
